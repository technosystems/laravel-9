					...
					if(productos.length == 0){
						...
						for (i = 0; i < productos.length; i++) { 
							...
							$("#listaBusquedaProducto").append(
								$('<option></option>').val(productos[i].codigo_barra).html(productos[i].descripcion + "," + productos[i].stock_actual + productos[i].presentacion + " disponibles.")
							);
						}
					}				
				});
	...
	if(data["productos"].length > 0){
		...
		if( productoBuscado == null){
			...
			if(producto_stock > 0){
				...
				var producto_precio = producto["precio_venta"]; 
				var producto_iva = 0;
